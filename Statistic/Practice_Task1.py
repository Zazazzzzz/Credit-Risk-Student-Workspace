...
# os.path.dirname gets the folder containing this script
# os.path.abspath gets the full absolute path
# This ensures we always look for files in the right place

credit_dir = os.path.join(current_dir, '..', '..', 'Credit Risk')
# Navigate up two levels (from Shazia/ to main folder)
# Then into Credit Risk/ folder where the Excel files live
# '..' means go up one folder level

index_as = pd.read_excel(
    os.path.join(credit_dir, 'EM Asia monthly.xls'),
    header=6
)
# Read the Asia equity index Excel file
# header=6 tells pandas that row 7 (index 6) contains column names

index_me = pd.read_excel(
    os.path.join(credit_dir, 'EM Europe and Middle East monthly.xls'),
    header=6
)
# Read the Europe/Middle East equity index Excel file
